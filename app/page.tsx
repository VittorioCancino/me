...
const contact = [
  {
    icon: Briefcase,
    label: "LinkedIn",
    href: "https://www.linkedin.com/in/vittorio-cancino-6163a5347/",
  },
  {
    icon: GitBranch,
    label: "VittorioCancino",
    href: "https://github.com/VittorioCancino",
  },
  {
    icon: Building2,
      ...
      <div className="flex gap-6">
        ...
        <aside className="w-72 shrink-0">
          <div className="overflow-hidden rounded-2xl border border-[#21262d] bg-[#161b22]">
            <div className="relative h-48 w-full bg-[#0d1117]">
              <Image
                src={`${BASE}/avatar.jpg`}
                alt="Vittorio Cancino"
                fill
                priority
                className="object-cover object-top"
              />
              <div className="absolute inset-x-0 bottom-0 h-16 bg-linear-to-t from-[#161b22] to-transparent" />
            </div>

            <div className="relative -mt-6 px-6 pb-6">
              <h1 className="text-xl leading-tight font-bold text-white">
                Vittorio Cancino Gonzalez
              </h1>
              <p className="mt-0.5 mb-1 text-sm font-medium text-[#7c3aed]">
                {t.sidebar.role}
              </p>
              <p className="mb-5 text-xs text-[#848d97]">
                {t.sidebar.pronouns}
              </p>

              <p className="mb-6 border-l-2 border-[#7c3aed] pl-3 text-sm leading-relaxed text-[#c9d1d9]">
                {t.sidebar.bio}
              </p>

              <div className="mb-6 border-t border-[#21262d]" />

              <ul className="space-y-3">
                {contact.map(({ icon: Icon, label, href }) => (
                  <li
                    key={label}
                    className="flex items-center gap-3 text-xs text-[#c9d1d9]"
                  >
                    ...
                    {href ? (
                      <a
                        ...
                        className="transition-colors hover:text-[#a78bfa] hover:underline"
                      >
                        {label}
                      </a>
                    ) : (
        ...
        <main className="min-w-0 flex-1 space-y-6">
          ...
          <section className="rounded-2xl border border-[#21262d] bg-[#161b22] px-7 py-6">
            <h2 className="mb-4 text-xs font-semibold tracking-widest text-[#7c3aed] uppercase">
              {w.title}
            </h2>
            <p className="mb-4 text-sm leading-relaxed text-[#c9d1d9]">
              {w.p1.split(w.highlights.sysadmin)[0]}
              <span className="font-medium text-white">
                {w.highlights.sysadmin}
              </span>
              {
                w.p1
                  .split(w.highlights.sysadmin)[1]
                  .split(w.highlights.infra)[0]
              }
              <span className="font-medium text-white">
                {w.highlights.infra}
              </span>
              ...
              <span className="font-medium text-white">
                {w.highlights.ansible}
              </span>
              {
                w.p1
                  .split(w.highlights.ansible)[1]
                  .split(w.highlights.declarative)[0]
              }
              <span className="font-medium text-white">
                {w.highlights.declarative}
              </span>
            ...
            <p className="mb-4 text-sm leading-relaxed text-[#c9d1d9]">
              ...
              <span className="font-medium text-white">
                {w.highlights.kubernetes}
              </span>
              {w.p2.split(w.highlights.kubernetes)[1].split(w.highlights.c)[0]}
              <span className="font-medium text-white">{w.highlights.c}</span>
              {w.p2.split(w.highlights.c)[1].split(w.highlights.go)[0]}
              <span className="font-medium text-white">{w.highlights.go}</span>
            ...
            <p className="mb-6 text-sm leading-relaxed text-[#c9d1d9]">
              ...
              <span className="font-medium text-white">{w.highlights.ai}</span>
              {w.p3.split(w.highlights.ai)[1]}
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-[#7c3aed]/40 bg-[#7c3aed]/10 px-3 py-0.5 text-xs text-[#a78bfa]"
                >
                  {tag}
                </span>
              ))}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-white">
                      {course.code}
                      <span className="ml-2 font-normal text-[#848d97]">
                        — {course.name}
                      </span>
                    </p>
                    <p className="mt-0.5 text-xs text-[#848d97]">
                      {course.description}
                    </p>
                  </div>
